...
const Customers = () => {
  ...
  return (
    <div className="flex flex-col md:h-screen md:overflow-hidden min-h-screen bg-slate-50/50 p-2 md:p-6 md:rounded-[40px] border border-slate-200/50 relative overflow-hidden font-sans">
      ...
      <div className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6 relative z-10 flex-shrink-0">
        <div className="text-center md:text-left">
          ...
          <h1 className="text-2xl md:text-4xl font-black text-gray-900 tracking-tight uppercase leading-none">VIP Reward Program</h1>
          <p className="text-xs text-gray-400 mt-2 font-bold uppercase tracking-[0.2em]">Rewarding top spenders (₹{threshold.toLocaleString()}+)</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative group">
                ...
                <input 
                    ...
                    className="h-12 md:h-14 bg-white border-2 border-transparent focus:border-brand-primary/30 rounded-2xl pl-12 pr-6 text-sm font-bold text-gray-900 shadow-xl shadow-gray-200/50 focus:outline-none transition-all w-full md:w-80"
                    value={searchQuery}
      ...
      <div className="flex flex-col lg:flex-row gap-8 relative z-10 flex-1 min-h-0 md:overflow-hidden">
        {/* Left Sidebar: Metrics */}
        <div className="w-full lg:w-80 flex flex-col gap-4 md:gap-6 md:overflow-y-auto custom-scrollbar pb-4 pr-2">
          <div className="bg-white rounded-[32px] p-6 shadow-xl shadow-gray-200/50 border border-gray-100 group hover:scale-[1.02] transition-all">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                  ...
                  <h2 className="text-2xl md:text-3xl font-black text-gray-900 leading-none">{vipCustomers.length}</h2>
              </div>
            </div>
          ...
          <div className="bg-white rounded-[32px] p-6 shadow-xl shadow-gray-200/50 border border-gray-100 group hover:scale-[1.02] transition-all">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                  ...
                  <h2 className="text-2xl md:text-3xl font-black text-gray-900 leading-none">{vipCustomers.reduce((acc, curr) => acc + curr.points, 0)}</h2>
              </div>
            </div>
        ...
        <div className="flex-1 bg-white rounded-[40px] shadow-2xl shadow-gray-200/50 border border-gray-100 overflow-hidden flex flex-col relative z-10 md:overflow-hidden">
          <div className="px-6 md:px-8 py-4 md:py-6 border-b border-gray-50 flex items-center justify-between bg-gray-50/30 flex-shrink-0">
              <h3 className="text-sm font-black uppercase tracking-widest text-gray-900 flex items-center gap-2">
                  <Users size={18} className="text-brand-primary" /> 
                  VIP Registry
              </h3>
              <span className="text-[9px] md:text-[10px] font-black text-sky-600 bg-sky-50 px-3 py-1 rounded-full uppercase tracking-tighter">
                  {vipCustomers.length} MEMBERS
              </span>
          </div>

          <div className="flex-1 overflow-auto custom-scrollbar">
            {vipCustomers.length > 0 ? (
                <div className="min-w-[600px] lg:min-w-0 overflow-x-auto">
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            {cust.userId ? (
                                                <button 
                                                    ...
                                                    className="h-8 md:h-9 px-3 md:px-4 bg-brand-primary text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-sky-700 transition-all flex items-center gap-2 shadow-lg shadow-brand-primary/10 active:scale-95 disabled:opacity-50"
                                                >
                                            ...
                                            <button 
                                                ...
                                                className="w-8 md:w-9 h-8 md:h-9 border-2 border-emerald-100 text-emerald-500 rounded-xl flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-all active:scale-90 shrink-0"
              ...
              <div className="h-full flex flex-col items-center justify-center p-10 md:p-20 text-center">
                  <div className="w-20 md:w-24 h-20 md:h-24 bg-gray-50 rounded-[40px] flex items-center justify-center mb-6">
                      <Users className="text-gray-200" size={48} />
                  </div>
                  <p className="text-xl font-black text-gray-900 mb-2 uppercase">No VIPs Yet</p>
                  <p className="text-sm text-gray-400 font-bold max-w-xs">Customers appear here after spending ₹{threshold.toLocaleString()} in your shop.</p>
              </div>
            )}
          </div>
          {vipCustomers.length > 0 && (
